...
> 💡 **코드 검토 시 참고 사항**
> 현재 `Program.cs`의 `ApplyState` 메서드를 보면, 안정성과 퍼포먼스를 위해 기본 화살표(`OCR_NORMAL`)와 텍스트 입력선(`OCR_IBEAM`)만 강제 교체하도록 활성화되어 있습니다. 십자 커서(`OCR_CROSS`)나 창 크기 조절 커서 등은 주석 처리되어 있으며, 필요에 따라 주석을 해제하여 윈도우의 모든 시스템 커서를 덮어씌울 수 있습니다.
...
> 💡 **레이어드 윈도우 숨김 최적화 트릭 (꿀팁)**
> 코드를 보면 인디케이터를 화면에서 숨길 때, 투명도를 조절하거나 메모리에서 객체를 파괴/재생성하지 않습니다. 대신 `Location = new Point(-10000, -10000)`처럼 윈도우를 화면 밖 아득히 먼 곳으로 날려버리는 트릭을 사용합니다. 이는 GDI 자원 할당/해제에 드는 오버헤드를 완전히 제거하는 매우 훌륭한 퍼포먼스 튜닝 기법입니다.
...
| 🔵 DeepSkyBlue | 하늘색 | `#00BFFF` | (0, 191, 255) | 영어 대문자 (현행) |
| 🔵 Blue | 파란색 | `#007AFF` | (0, 122, 255) | 영어 대문자 |
| 🟠 Orange | 주황색 | `#FF9500` | (255, 149, 0) | 빨리어(Pāḷi) 소문자 (현행) |
| 🟢 Lime | 라임색 | `#00FF00` | (0, 255, 0) | 빨리어(Pāḷi) 대문자 (현행) |
...
| 🔘 Black | 검정색 | `#000000` | (0, 0, 0) | 기타 다국어 확장용 |
...
문서 작업 시 마우스 화살표나 I-Beam(텍스트 입력선) 하단에 따라다니는 작은 원(Indicator)의 반경 크기 및 중심점 오프셋 좌표를 튜닝할 수 있습니다.

> 📍 **크기/모양 수정 위치:** `MainForm.cs` 내부의 `BakeIndicatorBuffer` 메서드
>
> 📍 **위치(오프셋) 수정 위치:** `MainForm.cs` 내부의 `StateTimer_Tick` 메서드 하단부

**1) 디자인 및 크기 조정**

원은 16×16 픽셀의 투명 캔버스 안에 그려집니다. `RectangleF`의 파라미터(시작 X, 시작 Y, 너비, 높이)를 조정하여 원의 크기를 변경합니다.

```csharp
// [예시] BakeIndicatorBuffer() 내부 - 원을 조금 더 크게 그리고 싶을 때
RectangleF rect = new RectangleF(2.0f, 2.0f, 12.0f, 12.0f); // X, Y, 너비, 높이
```

**2) 마우스 포인터와의 거리(위치) 제어**

실제 핫스팟(마우스 끝점)에서 원이 얼마나 떨어져서 따라다닐지는 타이머 루프 내에서 결정됩니다.

> 💡 <span style="color:#1F618D">**High-DPI 동적 스케일링 적용됨:**</span> `ratio` 변수는 사용자의 윈도우 설정(DPI 및 마우스 포인터 크기)에 따라 동적으로 변합니다. 단순히 픽셀을 고정하는 것이 아니라, 사용자의 화면 배율에 비례하여 인디케이터의 오프셋 거리도 자연스럽게 멀어지고 가까워집니다.

```csharp
// [예시] StateTimer_Tick() 내부 - 오프셋 값을 직접 제어
// pt.X 와 pt.Y 에 더해지는 값을 늘리면 원이 마우스에서 더 멀어집니다.
UpdateLayeredIndicator(_currentDotColor, pt.X + (int)(10 * ratio), pt.Y + (int)(30 * ratio));
```
...
윈도우 API(GetKeyboardLayout)를 통해 반환되는 자판 핸들(HKL)의 하위 16비트는 언어 식별자(LANGID)를 나타냅니다. 향후 새로운 언어 감지 로직을 추가할 때 조건문(if)에서 식별자로 활용할 주요 국가별 자판 코드입니다.
...
> 💡 **코드 검토 시 참고 사항**
> 기존 소스 코드에서 빨리어와 타 다국어를 동시에 판별할 때는 **상위 비트(Device ID)**와 **하위 비트(LANGID)**를 모두 추출하여 비교하는 구조를 취하게 됩니다.

**2️⃣ `ImeState.Detect` 내부의 비트 연산 및 조건식 확장**

기존 코드에서 `deviceId == 0xF0C0`만 체크하던 부분에 하위 16비트 검사 로직을 추가하고, OR(`||`) 연산자를 사용하여 중국어 등의 언어 ID를 함께 처리하도록 확장합니다.

> 💡 **비트 연산(Bit Shift) 해석 팁 (C++/VB 관점)**
> - `hklValue & 0xFFFF` : 64비트 숫자에서 마스크를 씌워 **하위 16비트(LANGID)**만 깔끔하게 빼내는 연산입니다.
> - `(hklValue >> 16) & 0xFFFF` : 숫자를 오른쪽으로 16칸 밀어버린 뒤 마스크를 씌워 **상위 16비트(특수 Device ID)**만 빼내는 연산입니다.

```csharp
// 입력기 핸들(HKL)을 64비트 정수로 가져오기
long hklValue = NativeMethods.GetKeyboardLayout(threadId).ToInt64();

// 1. 상위 16비트 추출 (빨리어 자판 등 커스텀 디바이스 식별용)
ushort devId = (ushort)((hklValue >> 16) & 0xFFFF);

// 2. 하위 16비트 추출 (일반 국가별 언어 식별용) - 신규 추가!
ushort langId = (ushort)(hklValue & 0xFFFF);

// 판별 로직 확장 (중국어 간체 0x0804 추가 예시)
if (devId == PALI_DEVICE_ID || langId == 0x0804)
{
    // 조건 통과 시, 기존 로직에 의해 캡스락 여부에 따라 라임색/주황색이 자동 반환됨
...
조건문을 통과한 언어들은 자동으로 하단의 Caps Lock 검사 로직을 거쳐 소문자(Orange) 상태와 대문자(Lime) 상태로 매핑됩니다. Graphic Asset 구조를 변경하지 않고 판별식 하나만 확장하여 결합도를 낮게 유지하는 최적화 설계입니다.
